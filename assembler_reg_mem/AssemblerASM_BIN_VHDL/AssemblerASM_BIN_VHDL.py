...
inputASM = 'ASM2.txt' #Arquivo de entrada de contém o assembly
...
#se der ruim botar pra false dnv
noveBits = True;
...
def  converteArroba(line):
    line = line.split('@')
    print(line)
    if line[1] not in dic_label.keys():
        line[1] = hex(int(line[1]))[2:].upper().zfill(2)
        line = ''.join(line)
    else:
        # if line[1]=="ATUALIZA":
        #     print("atualiza ta estranho")
...
def trataMnemonico(line):
    ...
    inst=mne[line[0]]
    if len(line)==3:
        test_line[0]=regs[line[1][:-1]]
        test_line[1]=mne[line[0]]
        test_line[2]=line[2]
...
with open(outputBIN, "w+") as f:  #Abre o destino BIN
    ...
    for line in lines:
        ...
        #Se a linha for válida para conversão, executa
            ...
            if ':' in line:
                #print(line.split(':'))
                label,comentario=defineLabel(line)
                dic_label[label]=cont
    ...
    for line in lines:        
        ...
        #Se a linha for válida para conversão, executa
            ...
            # print(instrucaoLine[:2])
            # print(instrucaoLine[2])
            for reg,v in regs.items():
                if v==instrucaoLine[:2]:
                    if not instrucaoLine[2].isdigit():
                        if instrucaoLine[2] not in mne.values():
                            instrucaoLine = reg + " " + "& "+ instrucaoLine[2:]

                        for inst,value in mne.items():
                            if value==instrucaoLine[2]:
                                instrucaoLine = reg + " " + "& "+ inst + instrucaoLine[3:]
            ...
            # print(instrucaoLine[:8]+instrucaoLine[9:])
            # print(len(instrucaoLine))

            if len(instrucaoLine)==22:
                instrucaoLine=instrucaoLine[:8]+" "+instrucaoLine[9:]
            if len(instrucaoLine)==21:
                instrucaoLine=instrucaoLine[:7]+" "+instrucaoLine[8:] 
            if len(instrucaoLine)==23:
                instrucaoLine=instrucaoLine[:9]+" "+instrucaoLine[10:]
